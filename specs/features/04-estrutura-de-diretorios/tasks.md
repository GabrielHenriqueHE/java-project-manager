...
Ainda fora de escopo: edição/exibição de `DirectoryNode.role` no checklist/árvore visual do Painel [5] (o dado agora está disponível no modelo de domínio via `source_dirs`/etc., mas a apresentação na TUI ainda não distingue "registrado via build-helper" de "convenção padrão"), remoção de diretório não-vazio, remoção de um nó qualquer da árvore (só os itens do checklist fixo são acionáveis), e desregistrar um diretório já registrado no build — ficam para fatias futuras.
