...
- Fases futuras, em ordem de prioridade:
  1. **Feature 03 — Gerenciamento de Módulos** (fatia seguinte): adicionar/editar dependência **direta** (`managed=False`) de um módulo qualquer pela TUI (hoje só o Painel [4]/BOM tem gancho de UI).
  2. **Feature 01 — Gerenciamento de Projetos** (fatia seguinte): clone remoto.
  3. **Suporte a Gradle** (novo `GradleAdapter` implementando `BuildToolAdapter`) — nenhum spec escrito ainda; fica por último porque depende da interface do adapter estar estável, e a fatia 1 ainda pode alterá-la.
