...
- **Fase 4 — Estrutura de Diretórios** (em andamento): `phases/phase-4-estrutura-diretorios/` (`BuildToolAdapter.add_directory`, novo método na interface, cria em disco qualquer item do checklist do Painel [5] ESTRUTURA que ainda não exista para o módulo ativo, sem tocar em `pom.xml`), `phases/phase-4-diretorio-customizado/` (binding `n` abre um formulário de caminho relativo livre, reaproveitando `add_directory` sem nenhuma mudança na camada de domínio) e `phases/phase-4-remover-diretorio/` (`BuildToolAdapter.remove_directory`, binding `d` no checklist, limitado a diretório vazio — nunca remove recursivamente).
- Fases futuras, em ordem de prioridade:
  1. **Feature 04 — Estrutura de Diretórios** (fatias seguintes): atribuir `DirectoryRole` a um diretório customizado (hoje sem onde persistir essa escolha entre sessões, já que o produto nunca tem manifesto próprio — decisão 4); remover diretório não-vazio ou um nó qualquer da árvore; registrar diretórios não-padrão no build (ex.: `build-helper-maven-plugin` para fontes extras).
