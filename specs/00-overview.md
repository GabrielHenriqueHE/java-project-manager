...
- **Fase 4 — Estrutura de Diretórios** (`phases/phase-4-estrutura-diretorios/`, em andamento): primeira fatia concluída — `BuildToolAdapter.add_directory` (novo método na interface) cria em disco qualquer item do checklist do Painel [5] ESTRUTURA que ainda não exista para o módulo ativo, sem tocar em `pom.xml`. Fatias futuras: caminho de diretório livre digitado pelo usuário, edição de `role`, remoção de diretórios.
