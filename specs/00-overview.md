...
- Fases futuras, em ordem de prioridade:
  1. **Feature 01 — Gerenciamento de Projetos** (fatia seguinte): clone remoto.
  2. **Feature 03 — Gerenciamento de Módulos** (fatia seguinte): remover uma dependência direta pela TUI (o `MavenPomWriter.remove_dependency` já existe, usado internamente por `remove_module`, mas não está exposto como operação de usuário).
  3. **Suporte a Gradle** (novo `GradleAdapter` implementando `BuildToolAdapter`) — nenhum spec escrito ainda; fica por último porque depende da interface do adapter estar estável, e as fatias 1–2 ainda podem alterá-la.
