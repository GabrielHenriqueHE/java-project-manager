...
- **Fase 4 — Estrutura de Diretórios** (em andamento): `phases/phase-4-estrutura-diretorios/` (`BuildToolAdapter.add_directory`, novo método na interface, cria em disco qualquer item do checklist do Painel [5] ESTRUTURA que ainda não exista para o módulo ativo, sem tocar em `pom.xml`) e `phases/phase-4-diretorio-customizado/` (binding `n` abre um formulário de caminho relativo livre, reaproveitando `add_directory` sem nenhuma mudança na camada de domínio).
- Fases futuras, em ordem de prioridade:
  1. **Feature 04 — Estrutura de Diretórios** (fatias seguintes): atribuir `DirectoryRole` a um diretório customizado (hoje sem onde persistir essa escolha entre sessões, já que o produto nunca tem manifesto próprio — decisão 4); remover diretórios; registrar diretórios não-padrão no build (ex.: `build-helper-maven-plugin` para fontes extras).
  2. **Feature 03 — Gerenciamento de Módulos** (fatias seguintes de `update_dependency`): remover uma dependência já declarada pela TUI; adicionar/editar dependência **direta** (`managed=False`) de um módulo qualquer pela TUI (hoje só o Painel [4]/BOM tem gancho de UI).
  3. **Feature 01 — Gerenciamento de Projetos** (fatias seguintes): criação de projeto do zero, clone remoto.
  4. **Suporte a Gradle** (novo `GradleAdapter` implementando `BuildToolAdapter`) — nenhum spec escrito ainda; fica por último porque depende da interface do adapter estar estável, e as fatias 1–3 ainda podem alterar essa interface (ex.: um eventual `remove_directory`/`remove_dependency`).
