...
- Fases futuras, em ordem de prioridade:
  1. **Feature 04 — Estrutura de Diretórios** (fatias seguintes): atribuir `DirectoryRole` a um diretório customizado na visualização/checklist (hoje sem onde persistir essa escolha entre sessões, já que o produto nunca tem manifesto próprio — decisão 4; o `role` escolhido em `phase-4-registrar-build` é gravado no `pom.xml`, não reabre essa decisão, mas não fica refletido de volta no checklist); remover diretório não-vazio ou um nó qualquer da árvore; desregistrar um diretório do build; ler de volta o `<build><plugins>` existente durante `infer_structure`.
