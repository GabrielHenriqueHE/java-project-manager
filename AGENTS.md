...
- `src/manager/adapters/base.py` — `BuildToolAdapter` ABC every build tool implements: `detect`, `infer_structure`, `add_module`, `remove_module`, `update_dependency`, `update_metadata`, `add_directory`. All seven are implemented end-to-end in `MavenAdapter`.
