...
- `src/manager/adapters/base.py` — `BuildToolAdapter` ABC every build tool implements: `detect`, `infer_structure`, `add_module`, `remove_module`, `update_dependency`, `update_metadata`. All six are implemented end-to-end in `MavenAdapter` (Phase 2 complete).
...
- `1`-`5`/`tab` switch panel focus, `j`/`k` move within a list, `n` create, `d` delete, `enter` select/edit, `space` toggle active module in Estrutura, `:` opens command mode (e.g. `:modulo <nome>`).
