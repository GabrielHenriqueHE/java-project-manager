...
`jpm` (Java Project Manager) is a **Python/Textual TUI** — not a Java tool. It manages the structure of Java projects (Maven and Gradle) from the outside: creating/removing modules, editing metadata, keeping the parent POM, BOM (`dependencyManagement`) and dependent modules in sync. The project itself is Python; "Java" refers to what it manages.
...
- `src/manager/adapters/base.py` — `BuildToolAdapter` ABC every build tool implements: `detect`, `infer_structure`, `create_project`, `add_module`, `remove_module`, `update_dependency`, `update_metadata`, `add_directory`, `remove_directory`, `remove_dependency`, `remove_direct_dependency`, `register_directory_role`, `unregister_directory_role` (11 methods). `MavenAdapter` implements all 11 end-to-end. `GradleAdapter` implements all of them except `register_directory_role`/`unregister_directory_role` (the Gradle equivalent, `sourceSets{}`, is a permanent scope boundary, not a pending stub — see `specs/features/06-suporte-gradle/`).
- `src/manager/adapters/maven/` — `adapter.py` orchestrates, `parser.py` reads POMs, `writer.py` edits them, `directory.py` handles module scaffolding on disk, `xml_utils.py` for namespace/XPath helpers. Uses `lxml` specifically to preserve formatting/comments in the user's POM and minimize diffs.
- `src/manager/adapters/gradle/` — `adapter.py` orchestrates, `parser.py` reads `build.gradle(.kts)`/`settings.gradle(.kts)` (Groovy and Kotlin DSL) via regex + brace-counting over a conventional subset of the language (no real Groovy/Kotlin parser), `writer.py` (`GradleWriter`) applies the same kind of surgical textual edits for mutations, reusing the parser's grammar so written text always parses back correctly.
- `src/manager/adapters/common/` — logic shared between adapters because it's genuinely build-tool-agnostic: `directory.py` (the `src/main/java` convention, identical between Maven and Gradle), `lookup.py` (pure tree-walk helpers over `Module`/`Dependency` — `find_module`, `find_parent`, `find_bom_module`, `find_dependents`, `find_managed_dependency_owner`, `resolve_module_relative_path`).
